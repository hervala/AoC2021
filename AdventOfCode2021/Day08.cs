...
namespace AdventOfCode2021
{
    ...
    public enum Segment
    {
        ...
        Segment5 = 32,
        Segment6 = 64,
        Value0 = Segment.Segment0 | Segment.Segment1 | Segment.Segment2 | Segment.Segment4 | Segment.Segment5 | Segment.Segment6,
        Value1 = Segment.Segment2 | Segment.Segment5,
        Value2 = Segment.Segment0 | Segment.Segment2 | Segment.Segment3 | Segment.Segment4 | Segment.Segment6,
        Value3 = Segment.Segment0 | Segment.Segment2 | Segment.Segment3 | Segment.Segment5 | Segment.Segment6,
        Value4 = Segment.Segment1 | Segment.Segment2 | Segment.Segment3 | Segment.Segment5,
        Value5 = Segment.Segment0 | Segment.Segment1 | Segment.Segment3 | Segment.Segment5 | Segment.Segment6,
        Value6 = Segment.Segment0 | Segment.Segment1 | Segment.Segment3 | Segment.Segment4 | Segment.Segment5 | Segment.Segment6,
        Value7 = Segment.Segment0 | Segment.Segment2 | Segment.Segment5,
        Value8 = Segment.Segment0 | Segment.Segment1 | Segment.Segment2 | Segment.Segment3 | Segment.Segment4 | Segment.Segment5 | Segment.Segment6,
        Value9 = Segment.Segment0 | Segment.Segment1 | Segment.Segment2 | Segment.Segment3 | Segment.Segment5 | Segment.Segment6,
    }

    [Command(nameof(Day08))]
    public class Day08 : DayCommand, ICommand
    {
        private Regex matcher = new Regex(@"(\w+)\s(\w+)\s(\w+)\s(\w+)", RegexOptions.Compiled);

        public override ValueTask Part01(IConsole console)
        {
            var input = ParseInput(Day08.input);
            var uniqueCnt = 0;

            foreach (var (patterns, value) in input)
            {
                ...
                if (match.Success)
                {
                    for (int i = 1; i < 5; i++)
                    {
                        uniqueCnt += match.Groups[i].Value.Length switch
                        {
                            2 or 3 or 4 or 7 => 1,
                            _ => 0,
                        };
            ...
            console.Output.WriteLine(uniqueCnt);
            return default;
        }

        public override ValueTask Part02(IConsole console)
        {
            var segment = new List<char>[7];
            var input = ParseInput(Day08.input);

            var total = 0;
            foreach (var (patternStr, valueStr) in input)
            {
                var patterns = patternStr.Trim().Split(' ');

                // 1
                var pattern1 = patterns.Single(o => o.Length == 2);
                segment[2] = new List<char> { pattern1[0], pattern1[1] };
                segment[5] = new List<char> { pattern1[0], pattern1[1] };

                // 7
                var pattern7 = patterns.Single(o => o.Length == 3);
                segment[0] = new List<char> { pattern7.First(c => !pattern1.Contains(c)) };

                // 4
                var segment1_7 = segment[2].Union(segment[0]);
                var pattern4 = patterns.Single(o => o.Length == 4);
                var charsFrom4 = pattern4.Where(c => !segment1_7.Contains(c)).ToArray();
                segment[1] = new List<char> { charsFrom4[0], charsFrom4[1] };
                segment[3] = new List<char> { charsFrom4[0], charsFrom4[1] };

                // 8
                var segment1_7_4 = segment[2].Union(segment[0]).Union(segment[1]);
                var pattern8 = patterns.Single(o => o.Length == 7);
                var charsFrom8 = pattern8.Where(c => !segment1_7_4.Contains(c)).ToArray();
                segment[4] = new List<char> { charsFrom8[0], charsFrom8[1] };
                segment[6] = new List<char> { charsFrom8[0], charsFrom8[1] };

                // find 0
                var pattern6seg = patterns.Where(o => o.Length == 6);
                var pattern0 = pattern6seg.Single(o => o.Contains(segment[1][0]) ^ o.Contains(segment[1][1]));
                if (pattern0.Contains(segment[1][0]))
                {
                    segment[1] = new List<char> { segment[1][0] };
                    segment[3] = new List<char> { segment[3][1] };
                }
                else
                {
                    segment[1] = new List<char> { segment[1][1] };
                    segment[3] = new List<char> { segment[3][0] };
                }

                // find 9
                var pattern9 = pattern6seg.Single(o => o.Contains(segment[4][0]) ^ o.Contains(segment[6][1]));
                if (pattern9.Contains(segment[4][0]))
                {
                    segment[4] = new List<char> { segment[4][1] };
                    segment[6] = new List<char> { segment[6][0] };
                }
                else
                {
                    segment[4] = new List<char> { segment[4][0] };
                    segment[6] = new List<char> { segment[6][1] };
                }

                // find 6
                var pattern6 = pattern6seg.Single(o => o.Contains(segment[2][0]) ^ o.Contains(segment[5][1]));
                if (pattern6.Contains(segment[2][0]))
                {
                    segment[2] = new List<char> { segment[2][1] };
                    segment[5] = new List<char> { segment[5][0] };
                }
                else
                {
                    segment[2] = new List<char> { segment[2][0] };
                ...
                foreach (var i in Enumerable.Range(0,4))
                {
                    ...
                    foreach (var chr in digits[i])
                    {
                        valFlag = segmentSequence.IndexOf(chr) switch
                        {
                            0 => valFlag | Segment.Segment0,
                            1 => valFlag | Segment.Segment1,
                            2 => valFlag | Segment.Segment2,
                            3 => valFlag | Segment.Segment3,
                            4 => valFlag | Segment.Segment4,
                            5 => valFlag | Segment.Segment5,
                            6 => valFlag | Segment.Segment6,
                            _ => throw new Exception("öö"),
                        };
                    }

                    digit = valFlag switch
                    {
                        Segment.Value0 => 0,
                        Segment.Value1 => 1,
                        Segment.Value2 => 2,
        ...
        private Segment7[] ParseInput(string input) =>
            input.Split(Environment.NewLine).Select(o =>
            {
                var parts = o.Split('|');
                return new Segment7(parts[0], parts[1]);
            }).ToArray();

        public record Segment7(string SignalPatterns, string OutputValue);

        private static string smallTestInput = @"acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf";
...
gcafb gcf dcaebfg ecagb gf abcdeg gaef cafbge fdbac fegbdc | fgae cfgab fg bagce";
        ...
        private static string input = @"ecdbfag deacfb acdgb cdg acdbf gdfb efacdg gd cagdbf beacg | cdg dcebgaf gbdf bdacg
